...
function getAliasesFromWebpackConfig(webpackConfigPath: string): Record<string, string> {
  try {
...
function getAliasesFromBabelConfig(babelConfigPath: string): Record<string, string> {
  try {
    const babelConfig = JSON.parse(fs.readFileSync(babelConfigPath, 'utf8'));
    const plugins = babelConfig.plugins || [];

    const moduleResolverPlugin = plugins.find((plugin: any) => Array.isArray(plugin) && plugin[0] === 'module-resolver');
...
function getAliasesFromVueConfig(vueConfigPath: string): Record<string, string> {
  try {
